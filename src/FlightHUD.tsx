...
import { BIOMES, RESONATORS } from './game/biomes'
...
export function WorldMap({ state, onSelect }: { state: Snapshot; onSelect: (id: number) => void }) {
  ...
  return <div className="world-map"><div className="map-chart"><svg viewBox="0 0 480 560" role="img" aria-label="6地区の全域マップ。右の地区一覧から目的地を選択できます。">
    ...
    {RESONATORS.map(r=><rect key={r.id} x={x(r.x)-2} y={z(r.z)-2} width="4" height="4" fill="#419492" />)}
    ...
    {DISTRICTS.map(d => <button key={d.id} disabled={state.zones[d.id].state === 'locked' || state.zones[d.id].state === 'cleared'} className={state.objective === d.id ? 'selected' : ''} onClick={() => onSelect(d.id)}>
      <b>{String(d.id + 1).padStart(2, '0')}</b><span>{d.name}<small>{labels[state.zones[d.id].state]} / {BIOMES[d.id].name}</small><small className="biome-boon">解放：{BIOMES[d.id].boon}</small></span><i>{state.zones[d.id].state === 'cleared' ? '✓' : '↗'}</i>
...
export function FlightHUD({ state, onPause, onMap }: { state: Snapshot; onPause: () => void; onMap: () => void }) {
  ...
  const biome = state.environment.zone >= 0 ? BIOMES[state.environment.zone] : null
  const eliteMax = state.bossMaxHp || state.wardenMaxHp, eliteHp = state.bossMaxHp ? state.bossHp : state.wardenHp
  return <div className={`hud frontier-hud ${state.overdrive > 0 ? 'overdrive-active' : ''} ${eliteMax > 0 ? 'has-boss' : ''}`}>
    ...
    {biome && <div className="biome-status" style={{ '--biome-color': biome.color } as CSSProperties}><span>LOCAL ENVIRONMENT <b>G ×{state.environment.gravity.toFixed(2)}</b></span><strong>{state.environment.phase}{state.environment.phaseRemaining>0&&<small> {Math.ceil(state.environment.phaseRemaining)}s</small>}</strong><p>{biome.rule}</p><div className="resonance-hint">◇ {biome.item}：{biome.itemHint}</div></div>}
    {state.buffs.length>0&&<div className="buff-status">{state.buffs.map(b=><span key={b.name}>{b.name} <b>{b.time.toFixed(1)}s</b></span>)}</div>}
    ...
    {eliteMax > 0 && <div className="boss-hud"><span>{state.bossMaxHp ? 'G–Ω / SOVEREIGN OF THE SKY' : 'WARDEN / ' + state.wardenName}</span><div><i style={{ width: `${eliteHp / eliteMax * 100}%` }} /></div><p>{state.bossMaxHp ? state.bossAttack || (state.bossHp < state.bossMaxHp / 2 ? 'OVERLOAD / PHASE II' : 'PHASE I') : state.wardenHint}</p></div>}
